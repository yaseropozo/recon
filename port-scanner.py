...
if __name__ == "__main__":
    ...
    if option in {"default", "top"}:
        ...
        target_ports = [80, 443] if option == "default" else [
            80, 443, 81, 300, 591, 593, 832, 981, 1010, 1311, 1099, 2082, 2095, 2096, 2480, 3000, 3001, 3002, 3003, 3128, 3333, 4243, 4567, 4711, 4712, 4993, 5000, 5104, 5108, 5280, 5281, 5601, 5800, 6543, 7000, 7001, 7396, 7474, 8000, 8001, 8008, 8014, 8042, 8060, 80                                                                                                     69, 8080, 8081, 8083, 8088, 8090, 8091, 8095, 8118, 8123, 8172, 8181, 8222, 8243, 8280, 8281, 8333, 8337, 8443, 8500, 8834, 8880, 8888                                                                                                     , 8983, 9000, 9001, 9043, 9060, 9080, 9090, 9091, 9092, 9200, 9443, 9502, 9800, 9981, 10000, 10250, 11371, 12443, 15672, 16080, 17778,                                                                                                      18091, 18092, 20720, 32000, 55440, 55672                                                                                                                                                                                                  
        ]
    elif option == "range":
        if len(sys.argv) != 4:
            print_usage_and_exit()
    ...
    try:
        ...
        for line in subdomains_list:
            ...
            for port in target_ports:
                try:
                    scanned_port, protocol = port_scan(ip_address, port)
                    print(f"{subdomain}:{scanned_port}:{protocol}")
                except TypeError:
                    pass
